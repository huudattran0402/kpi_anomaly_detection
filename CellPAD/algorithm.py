...
import numpy as np
from CellPAD.local_correlation_score import LCS
from CellPAD.smoothing_algorithms import WMA, EWMA, HW
...
class StatisticalPredictor:
    ...
    def predict(self, series, labels, period_len):
        ...
        """
        if self.predictor_name == "WMA" or self.predictor_name == "EWMA":
            predicted_series = self.predictor.predict_next_period(series, labels, period_len)
        if self.predictor_name == "HW":
...
class RegressionPredictor:
    ...
    def evaluate(self, X_train, X_test, y_train, y_test):
        return self.reg.score(X_train, y_train), self.reg.score(X_test, y_test)
    ##